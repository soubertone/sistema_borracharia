<?php	
	...
	$html .= '<legend id="ltitle">Relatório de Seviços por funcionário</legend>';
	...
	$results = "SELECT * FROM regservico WHERE codfuncionario IN('$codfuncionario')";
	...
	$html .= '</table>';
	$html .= '</fieldset>';
	$soma = "SELECT SUM(valortotal) FROM regservico WHERE codfuncionario IN('$codfuncionario');";
    $ins = mysqli_query($conn, $soma);
    while ($row = $ins->fetch_row() )  
    $valortotal = $row[0];
    $html .= '<label style="position: absolute; left: 600px; top: auto; margin-top: 3px;">Total: <label style="color: blue;">'.$valortotal."</label></label>";
